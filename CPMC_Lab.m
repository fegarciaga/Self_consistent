function [E_ave,E_err,Den_up_ave,Den_dn_ave,N_updn_ave, N_updn_err,savedFileName]=CPMC_Lab(Lx,Ly,Lz,N_up,N_dn,kx,ky,kz,U,tx,ty,tz,tx2,ty2,tz2,Den_up_ave,Den_dn_ave,deltau,N_wlk,N_blksteps,N_eqblk,N_blk,itv_modsvd,itv_pc,itv_Em,t_bp,t_pop,flag,flag_save,N_iter,suffix)
...
E=0;
W=0;
denup=zeros(N_sites,1);
dendn=zeros(N_sites,1);
% Preallocate arrays:
...
Den_up_blk=zeros(N_blk-N-1,N_sites);
Den_dn_blk=zeros(N_blk-N-1,N_sites);
N_updn_blk=zeros(N_blk-N-1,1);
...
for i_blk=1:N_eqblk
    for j_step=1:N_blksteps
        [Phi, Phi_old, B_up, B_dn, parents, w, O, denup, dendn, DD, E, W, W_bp] = stepwlk(Phi, Phi_old, B_up, B_dn, parents, N_wlk, N_sites, N, w, O, denup, dendn, DD, E, W, W_bp, H_k, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_up, N_par, t_bp, t_pop, U, Lx, Ly, fac_norm, aux_fld, ind_parents, ind_end, list);
...
for i_blk=1:N_blk
    for j_step=1:N_blksteps
        ...
        if ind_measure>0
            [Phi, Phi_old, B_up, B_dn, parents, w, O, Den_up_blk(ind_measure,:), Den_dn_blk(ind_measure,:), N_updn_blk(ind_measure), E_blk(i_blk), W_blk(i_blk), W_bp(ind_measure)] = stepwlk(Phi, Phi_old, B_up, B_dn, parents, N_wlk, N_sites, N, w, O, Den_up_blk(ind_measure,:), Den_dn_blk(ind_measure,:), N_updn_blk(ind_measure), E_blk(i_blk), W_blk(i_blk), W_bp(ind_measure), H_k, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_up, N_par, t_bp, t_pop, U, Lx, Ly, fac_norm, aux_fld, ind_parents+1, ind_end+1, list);
        else
            [Phi, Phi_old, B_up, B_dn, parents, w, O, denup, dendn, DD, E_blk(i_blk), W_blk(i_blk), W] = stepwlk(Phi, Phi_old, B_up, B_dn, parents, N_wlk, N_sites, N, w, O, denup, dendn, DD, E_blk(i_blk), W_blk(i_blk), W, H_k, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_up, N_par, t_bp, t_pop, U, Lx, Ly, fac_norm, aux_fld, ind_parents+1, ind_end+1, list);
        end
        if mod(j_step,itv_modsvd)==0
            [Phi, O] = stblz(Phi, N_wlk, O, N_up, N_par); % re-orthonormalize the walkers
        end
    ...
    if ind_measure>0        
       Den_up_blk(ind_measure,:)=Den_up_blk(ind_measure,:)/W_bp(ind_measure);
       Den_dn_blk(ind_measure,:)=Den_dn_blk(ind_measure,:)/W_bp(ind_measure);
       N_updn_blk(iind_measure)=N_updn_blk(ind_measure)/W_bp(ind_measure);
    end
...
Den_up=real(Den_up_blk);
Den_up_ave=mean(Den_up);
Den_dn=real(Den_dn_blk);
Den_dn_ave=mean(Den_dn);
N_updn=real(N_updn_blk);
N_updn_ave=mean(N_updn)
N_updn_err=std(N_updn)/sqrt(N_blk-N-1)
