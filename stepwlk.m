function [phi, phi_old, B_up, B_dn, parents, w, O, Den_up_blk, Den_dn_blk, N_updn_blk, E, W, W_bp] = stepwlk(phi, phi_old, B_up, B_dn, parents, N_wlk, N_sites, N, w, O, Den_up_blk, Den_dn_blk, N_updn_blk, E, W, W_bp, H_k, Proj_k_half, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_up, N_par, t_bp, t_pop, U, Lx, Ly, fac_norm, aux_fld, ind_parents, ind_end, list)
    ...
    Den_up=zeros(N_wlk,N_sites);
    Den_dn=zeros(N_wlk,N_sites); % Array containing a given operator
    N_updn=zeros(N_wlk,1);
            ...
            if w(i_wlk)>0
                ...
                if w(i_wlk)>0
                    ...
                    if flag_bp==1
                        ...
                        [Den_up(i_wlk,:), Den_dn(i_wlk,:),N_updn(i_wlk)]=measure_bp(Phi_T, phi_old(:,:,i_father,ind_end), B_up(:,i_wlk,:, ind_end), B_dn(:,i_wlk,:, ind_end), Proj_k_half, t_bp, t_pop, N_up, N_par, N_sites);
                    end
                end
            end
        end
    ...
    if flag_bp==1
        for i_wlk=1:N_wlk
            if w(i_wlk)>0
                Den_up_blk=Den_up_blk+Den_up(i_wlk,:)*w(i_wlk);
                Den_dn_blk=Den_dn_blk+Den_dn(i_wlk,:)*w(i_wlk);
                N_updn_blk=N_updb_blk+N_updn(i_wlk)*w(i_wlk);
                W_bp=W_bp+w(i_wlk);
            end
        end
        B_up(:,:,:,ind_end)=0;
